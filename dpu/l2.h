...
static inline void L2_init(L2_insert_task *sit) {
...
static inline void L2_build_lr(int height, pptr addr, pptr val, int64_t chk) {
    IN_DPU_ASSERT(addr.id == DPU_ID, "L2 build lr: wrong id");
    mL2ptr nn = (mL2ptr)addr.addr;
    if (height >= 0) {
        IN_DPU_ASSERT(height < nn->height, "L2 build lr: wrong height");
        nn->right[height] = val;
        nn->chk[height] = chk;
    } else {
        height = -1 - height;
...
static inline void L2_remove_parallel(int length, int64_t *keys,
                                      int32_t *oldnode_size, int32_t *oldnode_count) {
    uint32_t tasklet_id = me();
    ...
    for (int i = 0; i < length; i ++) {
        ...
        IN_DPU_ASSERT(nodes[i]->height <= LOWER_PART_HEIGHT, "L2 remove parallel: wrong height");
    ...
    __mram_ptr void* send_tasks = (__mram_ptr void*)(send_buffer + sizeof(int64_t) * (2 + oldnode_count[NR_TASKLETS - 1])); 
    ...
    for (int i = 0; i < length; i ++) {
        ...
        pptr* right = buffer + sizeof(L2_remove_reply) + sizeof(pptr) * heights[i];
        for (int ht = 0; ht < heights[i]; ht ++) {
            left[ht] = nodes[i]->left[ht];
            right[ht] = nodes[i]->right[ht];
        }
        send_offsets[l + i] = buffer_cnt;
        IN_DPU_ASSERT((void*)(&send_offsets[l + i]) < send_tasks, "L2 remove: wrong offset");
